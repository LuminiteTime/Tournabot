...
- **Раундовое расписание** — матчи выстраиваются по оптимальному порядку раундов; доступные матчи помечаются эмодзи
- **Интерактивная сетка** — вся таблица прямо в одном Telegram-сообщении в виде инлайн-клавиатуры
- **Ввод счётов** — нажал на доступный матч → матч начат (эмодзи-статус) → нажал снова → ввёл счёт → таблица обновилась
- **Навигация между таблицами** — кнопки ◀ / ▶ для переключения
- **Подтверждение завершения** — при незавершённых матчах подтверждение через инлайн-кнопки Да/Нет
...
`LOG_LEVEL` зафиксирован в коде (`INFO`).

> Observability-стек (Grafana/Loki/Prometheus) и метрики временно отключены.
